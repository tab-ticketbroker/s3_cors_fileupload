module S3CorsFileupload
  module FormHelper
    ...
    def s3_cors_fileupload_form_tag(options = {}, &block)
      ...
      construct_form_html(hidden_form_fields, policy_helper.options[:bucket], policy_helper.options[:region], options[:secure], options[:upload_buttons], options[:upload_body], options[:multiple], _html_options,  &block)
    end
    ...
    def construct_form_html(hidden_fields, bucket, region = "s3", secure = true, upload_button_bar = "", upload_body = "", multiple = true, html_options = {}, &block)
      ...
      form_tag(secure == false ? "http://#{region}.amazonaws.com/#{bucket}" : "https://#{region}.amazonaws.com/#{bucket}", build_form_options(html_options)) do
        hidden_fields.map do |name, value|
          hidden_field_tag(name, value)
              ...
              file_field_tag(:file, :multiple => multiple) + upload_body.html_safe + (block ? capture(&block) : '')
      end
    end
  end
end
